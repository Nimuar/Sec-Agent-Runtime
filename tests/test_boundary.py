...
class TestSimulationHarness(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sandbox_dir = os.path.abspath("runtime/sandbox")
    ...
    def tearDownClass(cls):
        ...
        avg_latency = sum(cls.metrics["latencies"]) / max(
            1, len(cls.metrics["latencies"])
        )
    ...
    def test_happy_path_read_file(self):
        ...
        resp = self._execute_and_track(payload)
        if resp.get("outcome") != "SUCCESS":
            print("READ_FAIL:", resp)
        self.assertEqual(resp.get("outcome"), "SUCCESS")
